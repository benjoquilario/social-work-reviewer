...
**Version 5 — 1 September 2026.** The current document. v1, v2, v3 and v4 are
kept beside it; where any of them disagree, this one is right.

### What changed in v5

v4 rebuilt identity, privacy and the community, and closed with the line
*"nothing changed about … billing."* v5 is the other half: billing is the only
thing that changed.

The trigger was an audit ahead of wiring up Play Billing, and it found the flow
could not have worked. `subscriptions.purchaseToken` — the column both purchase
handlers look rows up by, and the only thing a renewal notification from Google
carries — **had no index**. Appwrite needs one to filter on a column, so the
first real purchase would have failed at the moment access is granted, which is
to say after the member had paid.

That is fixed, along with five other things found beside it. All of them are
server-side. **The app has almost nothing to do.**

**Sections 1–23 keep their numbers**, as they have since v2. v5 appends
sections 24–25 and edits 6, 12 and 15 in place.

#### Nothing breaks at runtime. Two things break at compile time.

No column was renamed or removed, no permission narrowed, no query invalidated.
Every screen that works against v4 works against v5 unchanged. But two enums
**gained values**, and in TypeScript a widened union breaks anything exhaustive
over it:

| | What breaks | What to do |
| --- | --- | --- |
| **1** | `subscriptions.status` gained `in_grace_period`, `on_hold` and `paused`. A `Record<SubscriptionStatus, …>` or a `switch` with no `default` stops compiling. | Handle the three, or add a fallback — [section 24](#24-billing-in-v5) |
| **2** | `user_activity_log.type` gained `subscription_refunded`. Same shape of break in a type → icon map. | Same. The timeline renders `title` directly, so a fallback icon is enough |

`user_profiles.subscriptionStatus` — the field **every** paywall actually gates
on — is untouched and still has its five values. That is deliberate, and it is
why the blast radius is two type errors rather than every screen in the app.

#### Everything else

| | |
| --- | --- |
| **Fixed: the missing index** | `idx_subscription_token`, unique on `purchaseToken`. Billing could not have worked without it |
| **Fixed: a failed card kept full access** | Play's `ON_HOLD`, `IN_GRACE_PERIOD` and `PAUSED` notifications were silently ignored. Now they are three real states — [section 24](#24-billing-in-v5) |
| **Fixed: an upgrade left two subscriptions** | Play issues a *new* token on an upgrade and does not close the old one. `linkedPurchaseToken` is how the server now finds it and closes it |
| **New table** | `billing_notifications` — every message Google sent, keyed for exactly-once handling. Server-only; the app never reads it |
| **New columns** | Six on `subscriptions`, one on `payments`, one on `subscription_plans` — all read-only, all optional |
| **New section 24** | [Billing in v5](#24-billing-in-v5) — the grace period, and the one screen that should care |
| **New section 25** | [Upgrading the app from v4](#25-upgrading-the-app-from-v4) — the checklist, and it is short |

Nothing changed about questions, materials, subjects, sessions, answers,
progress, settings, the community, privacy or permissions. **If your screen does
not read a subscription row, v5 does not affect it at all.**
...
24. [Billing in v5](#24-billing-in-v5) — **new in v5** — the grace period, and the one screen that should care
25. [Upgrading the app from v4](#25-upgrading-the-app-from-v4) — **new in v5** — the checklist, and it is short

New to the project? Read 5 first — every item in it is a silent failure, not an error message.

Coming from v4 and only want the delta? Read 24, then 25. Nothing else moved.
...
| `subscriptions` | `idx_subscription_token` | unique | `purchaseToken` | The purchase token *is* the purchase. Every notification and every re-report from the app arrives carrying nothing else, so this is the lookup billing cannot work without — and unique is what makes applying the same purchase twice update one row instead of granting two. |
| `subscriptions` | `idx_subscription_linked_token` | key | `linkedPurchaseToken` | Finds the row an upgrade replaced, so the old subscription is closed instead of running alongside the new one. |
...
Three other things were fixed with it, all invisible from the app: Play's
